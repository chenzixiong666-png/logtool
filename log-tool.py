...
class LogToolApp:
    def __init__(self, root):
        ...
        button_frame1.pack(fill=tk.X, pady=(8, 2))
        ...
        button_frame2.pack(fill=tk.X, pady=(2, 8))
        ...
        help_text = (
            ...
            "📥 近期日志：仅抓取昨天和今天更新的日志\n"
            "🗂 全部日志：抓取设备上的全部日志文件\n"
            "    （以上两项同时拉取 aihub / central 的 mmkv.default 配置文件）\n"
            "🐛 收集系统日志：执行 adb bugreport，导出系统级压缩日志\n"
            "🔊 音频调试：设置Audio日志为VERBOSE并抓取音频/通知快照\n"
            "    使用方法：复现前点一次(设级别+基准快照) → 复现问题 → 再点一次(对比快照) → 拉日志"
        )
    ...
    def audio_debug(self):
        """音频调试 - 设置verbose日志级别 + 抓取音频/通知/系统设置快照"""
        save_path = self.save_path
        ...
        timestamp = self.get_bjt_now().strftime("%Y-%m-%d_%H-%M-%S")
        audio_dir = os.path.join(save_path, f"audio_debug_{timestamp}")
        
        self.update_status("【音频调试】正在设置日志级别并抓取快照...")
        self.update_status(f"  保存目录: audio_debug_{timestamp}")
        self.audio_btn.config(state=tk.DISABLED)
        
        thread = threading.Thread(
            target=self._run_audio_debug,
            args=(audio_dir,),
            daemon=True
        )
        thread.start()
    
    def _run_audio_debug(self, audio_dir):
        """后台执行音频调试命令"""
        try:
            os.makedirs(audio_dir, exist_ok=True)
            
            # Step 1: 设置 verbose 日志级别
            self.update_status("  [1/4] 设置 Audio 日志级别为 VERBOSE...")
            setprop_cmds = [
            ...
            setprop_results = []
            for cmd in setprop_cmds:
                result = subprocess.run(
                    cmd, shell=True, capture_output=True, text=True, timeout=10
                )
                tag = cmd.split("log.tag.")[1].split()[0]
                status = "✓" if result.returncode == 0 else "✗"
                setprop_results.append(f"{status} {tag}: rc={result.returncode}")
                self.update_status(f"    {status} {tag}")
            
            # 保存 setprop 结果
            with open(os.path.join(audio_dir, "setprop_result.txt"), "w", encoding="utf-8") as f:
                f.write("Audio Verbose 日志级别设置结果\n")
                f.write("=" * 40 + "\n")
                for r in setprop_results:
                    f.write(r + "\n")
                f.write("\n提示: VERBOSE级别已生效，logcat中将包含详细Audio日志\n")
            
            # Step 2: dumpsys audio
            self.update_status("  [2/4] 抓取 dumpsys audio...")
            result = subprocess.run(
                "adb shell dumpsys audio",
                shell=True, capture_output=True, text=True, timeout=30
            )
            audio_file = os.path.join(audio_dir, "dumpsys_audio.txt")
            with open(audio_file, "w", encoding="utf-8") as f:
                f.write(result.stdout or "")
            size_kb = os.path.getsize(audio_file) / 1024
            self.update_status(f"    ✓ dumpsys_audio.txt ({size_kb:.1f} KB)")
            
            # Step 3: dumpsys notification
            self.update_status("  [3/4] 抓取 dumpsys notification...")
            result = subprocess.run(
                "adb shell dumpsys notification",
                shell=True, capture_output=True, text=True, timeout=30
            )
            notif_file = os.path.join(audio_dir, "dumpsys_notification.txt")
            with open(notif_file, "w", encoding="utf-8") as f:
                f.write(result.stdout or "")
            size_kb = os.path.getsize(notif_file) / 1024
            self.update_status(f"    ✓ dumpsys_notification.txt ({size_kb:.1f} KB)")
            
            # Step 4: settings list system
            self.update_status("  [4/4] 抓取 settings list system...")
            result = subprocess.run(
                "adb shell settings list system",
                shell=True, capture_output=True, text=True, timeout=15
            )
            settings_file = os.path.join(audio_dir, "system_settings.txt")
            with open(settings_file, "w", encoding="utf-8") as f:
                f.write(result.stdout or "")
            size_kb = os.path.getsize(settings_file) / 1024
            self.update_status(f"    ✓ system_settings.txt ({size_kb:.1f} KB)")
            
            self.update_status(f"✓ 音频调试快照完成: {audio_dir}")
            self.update_status("  提示: 复现问题后可再次点击抓取对比快照")
            
            self.root.after(0, lambda: messagebox.showinfo(
                "音频调试完成",
                "已完成以下操作:\n\n"
                "1. 设置 AudioService/AudioFlinger/AudioPolicyManager 为 VERBOSE\n"
                "2. 抓取 dumpsys audio（音频路由、音量、焦点）\n"
                "3. 抓取 dumpsys notification（通知通道状态）\n"
                "4. 抓取 settings list system（系统音量设置）\n\n"
                "▶ 下一步：复现音频问题后再点一次，然后拉取日志对比分析"
            ))
            ...
            self.update_status("✗ 执行超时")
            self.root.after(0, lambda: (
                self.audio_btn.config(state=tk.NORMAL),
                messagebox.showerror("错误", "执行超时，请检查设备连接")
            ))
        except Exception as e:
            self.update_status(f"✗ 执行失败: {str(e)}")
            self.root.after(0, lambda: (
